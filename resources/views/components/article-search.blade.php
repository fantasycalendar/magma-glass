<div class="mx-5 pt-4 relative">
    ...
    <input type="search" name="email" id="email"
           ...
           @keyup.enter="if(searchResults.length) { updateArticle(searchResults[0].path); $event.srcElement.blur(); showSearchResults = false; }"
           @click.outside="$el.blur(); showSearchResults = false;"
    ...
    <div class="absolute bg-white border dark:bg-gray-800 dark:border-gray-600 rounded shadow-lg w-full" x-show.transition="showSearchResults">
        <ul>
            <template x-for="result in searchResults">
                <li class="px-4 py-2 cursor-pointer text-gray-500 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600" @click.prevent="updateArticle(result.path); $nextTick(() => showSearchResults = false)" x-text="result.title"></li>
            </template>
